...
import precision_tool.tf_config as npu_tf_config  # 导入工具
...
#             clf_h = npu_ops.dropout(clf_h, clf_pdrop, shape)
...
#         with tf.device(assign_to_gpu(i, "/gpu:0")), tf.variable_scope(tf.get_variable_scope(), reuse=do_reuse):
        with tf.variable_scope(tf.get_variable_scope(), reuse=do_reuse):
...
# # 训练迭代循环下沉-2
# train_op = util.set_iteration_per_loop(sess, train, 1)
